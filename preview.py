...
for annotation in os.listdir( annotations_dir ):
    ...
    for bounding in range( 0, len( bbox ) ):
        ...
        draw.rectangle( ( int( bbox[ bounding ][ "bndbox" ][ "xmin" ] ), int( bbox[ bounding ][ "bndbox" ][ "ymin" ] ), int( bbox[ bounding ][ "bndbox" ][ "xmax" ] ), int( bbox[ bounding ][ "bndbox" ][ "ymax" ] ) ), outline=outlineColor)
